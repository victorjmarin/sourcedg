...
public class Util {
  ...
  public static String criaPaginacao(String pagina, int numPag, int qtdeporpagina, int total) {
    ...
    for (int i = comeco; i <= fim; i++) {
      if (i == numPag) {
        ...
        paginacao += "<a style='text-decoration:underline' title='P�g. " + i
            + "' href=\"Javascript:navegacao('" + pagina + "'," + i + ")\">" + i + "</a> ";
      }
    }
    if (total == 0) {
  ...
  public static String converteDHWtoPNG(String caminhoupload, String caminhoperl,
      String caminhoj2sdk, String nomearquivo) {
    try {
      String programa1[] = {caminhoperl + "perl.exe", caminhoupload + "dhw2ps.pl", "--ps",
          caminhoupload + nomearquivo};
      String programa2[] = {caminhoj2sdk + "java", "-cp", caminhoupload + "digimemo.jar",
          "DigiPSToPng", caminhoupload, nomearquivo};
