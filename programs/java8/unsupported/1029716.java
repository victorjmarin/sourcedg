...
public class GlobalPlatformService implements ISO7816, APDUListener {
     ...
     */
    public GlobalPlatformService(AID aid, CardChannel channel) throws IllegalArgumentException {
     ...
     *             null.
     */
    public GlobalPlatformService(AID aid, CardChannel channel, int scpVersion) throws IllegalArgumentException {
     ...
     *             if {@code channel} is null.
     */
    public GlobalPlatformService(CardChannel channel) throws IllegalArgumentException {
     ...
     *             null.
     */
    public GlobalPlatformService(CardChannel channel, int scpVersion) throws IllegalArgumentException {
    ...
    public void exchangedAPDU(CommandAPDU c, ResponseAPDU r) {
        GPUtil.debug("Command  APDU: " + GPUtil.byteArrayToString(c.getBytes()));
        GPUtil.debug("Response APDU: " + GPUtil.byteArrayToString(r.getBytes()));
    }
    ...
    public void open() throws GPSecurityDomainSelectionException, CardException {
        if (sdAID == null) {
            ...
            for (Map.Entry<String, AID> entry : AID.SD_AIDS.entrySet()) {
                CommandAPDU command = new CommandAPDU(CLA_ISO7816, INS_SELECT, 0x04, 0x00, entry.getValue().getBytes());
                ...
                System.out.println("Failed to select Security Domain " + entry.getKey() + " " + entry.getValue().toString() + ", SW: " + GPUtil.swToString(sw));
            }
            if (sdAID == null) {
                throw new GPSecurityDomainSelectionException(sw, "Could not select any of the known Security Domains!");
            }
        } else {
            CommandAPDU command = new CommandAPDU(CLA_ISO7816, INS_SELECT, 0x04, 0x00, sdAID.getBytes());
            ResponseAPDU resp = channel.transmit(command);
            notifyExchangedAPDU(command, resp);
            short sw = (short) resp.getSW();
            if (sw != SW_NO_ERROR) {
                throw new GPSecurityDomainSelectionException(sw, "Could not select custom sdAID " + sdAID + ", SW: " + GPUtil.swToString(sw));
            }
        }
    }
    ...
    public void openSecureChannel(int keySet, int keyId, int scpVersion, int securityLevel, boolean gemalto) throws IllegalArgumentException, CardException {
        ...
        if (sw != SW_NO_ERROR) {
            throw new CardException("Wrong initialize update, SW: " + GPUtil.swToString(sw));
        }
        ...
        byte[] myCryptogram = GPUtil.mac_3des(sessionKeys.keys[0], GPUtil.pad80(bo.toByteArray()), new byte[8]);
        ...
        byte[] authData = GPUtil.mac_3des(sessionKeys.keys[0], GPUtil.pad80(bo.toByteArray()), new byte[8]);
        ...
        if (sw != SW_NO_ERROR) {
            throw new CardException("External authenticate failed. SW: " + GPUtil.swToString(sw));
        }
    ...
    private KeySet deriveSessionKeysSCP01(KeySet staticKeys, byte[] hostRandom, byte[] cardResponse) throws CardException {
        ...
        try {
            ICipher cipher = ICipher.Factory.getImplementation(ICipher.DESEDE_ECB_NOPADDING);
    ...
    private KeySet deriveSessionKeysSCP02(KeySet staticKeys, byte seq1, byte seq2, boolean implicitChannel) throws CardException {
        ...
        try {
            ...
            ICipher cipher = ICipher.Factory.getImplementation(ICipher.DESEDE_CBC_NOPADDING, GPUtil.getKey(staticKeys.keys[1], 24), new byte[8]);
    ...
    public ResponseAPDU transmit(CommandAPDU command) throws IllegalStateException, CardException {
        if (wrapper == null && (scpVersion == SCP_02_0A || scpVersion == SCP_02_0B || scpVersion == SCP_02_1A || scpVersion == SCP_02_1B)) {
            ...
            int keySet = 0;
            if (result.length > 6) keySet = result[result[0] != 0 ? 5 : 6];
            ...
            if (sw != SW_NO_ERROR) {
                throw new CardException("Reading sequence counter failed. SW: " + GPUtil.swToString(sw));
            }
            try {
                KeySet sessionKeys = deriveSessionKeysSCP02(staticKeys, result[2], result[3], true);
                byte[] temp = GPUtil.pad80(sdAID.getBytes());
                byte[] icv = GPUtil.mac_des_3des(sessionKeys.keys[1], temp, new byte[8]);
                byte[] ricv = GPUtil.mac_des_3des(sessionKeys.keys[3], temp, new byte[8]);
     ...
     *             for low-level communication problems
     */
    public void loadCapFile(CapFile cap, boolean includeDebug, boolean separateComponents, int blockSize, boolean loadParam, boolean useHash) throws GPInstallForLoadException, GPLoadException, CardException {
        byte[] hash = useHash ? cap.getLoadFileDataHash(includeDebug) : new byte[0];
        int len = cap.getCodeLength(includeDebug);
        byte[] loadParams = loadParam ? new byte[] { (byte) 0xEF, 0x04, (byte) 0xC6, 0x02, (byte) ((len & 0xFF00) >> 8), (byte) (len & 0xFF) } : new byte[0];
        ByteArrayOutputStream bo = new ByteArrayOutputStream();
        try {
        ...
        short sw = (short) response.getSW();
        if (sw != SW_NO_ERROR) {
            throw new GPInstallForLoadException(sw, "Install for Load failed, SW: " + GPUtil.swToString(sw));
        }
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            sw = (short) response.getSW();
            if (sw != SW_NO_ERROR) {
                throw new GPLoadException(sw, "Load failed, SW: " + GPUtil.swToString(sw));
            }
        }
    }
     ...
     */
    public void installAndMakeSelecatable(AID packageAID, AID appletAID, AID instanceAID, byte privileges, byte[] installParams, byte[] installToken) throws GPMakeSelectableException, CardException {
        ...
        short sw = (short) response.getSW();
        if (sw != SW_NO_ERROR) {
            throw new GPMakeSelectableException(sw, "Install for Install and make selectable failed, SW: " + GPUtil.swToString(sw));
        }
    }
     ...
     */
    public void deleteAID(AID aid, boolean deleteDeps) throws GPDeleteException, CardException {
        ByteArrayOutputStream bo = new ByteArrayOutputStream();
        try {
            bo.write(0x4f);
        ...
        short sw = (short) response.getSW();
        if (sw != SW_NO_ERROR) {
            throw new GPDeleteException(sw, "Deletion failed, SW: " + GPUtil.swToString(sw));
        }
    }
    ...
    public AIDRegistry getStatus() throws CardException {
        ...
        for (int p1 : p1s) {
            ...
            while (response.getSW() == 0x6310) {
                ...
                sw = (short) response.getSW();
                if (sw != SW_NO_ERROR && sw != (short) 0x6310) {
                    throw new CardException("Get Status failed, SW: " + GPUtil.swToString(sw));
                }
            }
            int index = 0;
            ...
            while (index < data.length) {
                ...
                if (p1 == 0x40) {
                    if ((privileges & 0x80) == 0) kind = AIDRegistryEntry.Kind.Application; else kind = AIDRegistryEntry.Kind.SecurityDomain;
                }
        ...
        for (int p1 : p1s) {
            ...
            while (index < data.length) {
                ...
                AID aid = new AID(data, index, len);
                index += len;
                AIDRegistryEntry entry = new AIDRegistryEntry(aid, data[index++], data[index++], p1 == 0x10 ? AIDRegistryEntry.Kind.ExecutableLoadFilesAndModules : AIDRegistryEntry.Kind.ExecutableLoadFiles);
                if (p1 == 0x10) {
                    int num = data[index++];
                    for (int i = 0; i < num; i++) {
                        len = data[index++];
    ...
    public class SecureChannelWrapper {
            ...
            try {
                ...
                if (mac) {
                    if (icv == null) {
                        ...
                        ICipher c = null;
                        if (scp == 1) {
                            c = ICipher.Factory.getImplementation(ICipher.DESEDE_ECB_NOPADDING, GPUtil.getKey(sessionKeys.keys[1], 24));
                        } else {
                            c = ICipher.Factory.getImplementation(ICipher.DES_ECB_NOPADDING, GPUtil.getKey(sessionKeys.keys[1], 8));
                        }
                        icv = c.encrypt(icv);
                    }
                    if (preAPDU) {
                    ...
                    t.write(origData);
                    if (scp == 1) {
                        icv = GPUtil.mac_3des(sessionKeys.keys[1], GPUtil.pad80(t.toByteArray()), icv);
                    } else {
                        icv = GPUtil.mac_des_3des(sessionKeys.keys[1], GPUtil.pad80(t.toByteArray()), icv);
                    }
                    if (postAPDU) {
                        newCLA = setBits((byte) newCLA, (byte) 0x04);
                        newLc = newLc + 8;
                ...
                if (enc && origLc > 0) {
                    ...
                    ICipher c = ICipher.Factory.getImplementation(ICipher.DESEDE_CBC_NOPADDING, GPUtil.getKey(sessionKeys.keys[0], 24), new byte[8]);
        ...
        private ResponseAPDU unwrap(ResponseAPDU response) throws CardException {
            if (rmac) {
                ...
                ricv = GPUtil.mac_des_3des(sessionKeys.keys[3], GPUtil.pad80(rMac.toByteArray()), ricv);
    ...
    public static void loadJCOPProvider() throws InstantiationException, ClassNotFoundException, IllegalAccessException, NoSuchAlgorithmException {
    ...
    public static void usage() {
        ...
        System.out.println(" -loadsize <num>   load block size, default " + defaultLoadSize);
    ...
    public static void main(String[] args) throws IOException {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (args[i].equals("-list")) {
                    ...
                    for (int k = 0; k < keysOpt.length; k++) {
                        if (args[i].equals(keysOpt[k])) index = k;
                    }
                    if (index >= 0) {
                        i++;
        ...
        try {
            TerminalFactory tf;
            if (use_jcop_emulator == false) tf = TerminalFactory.getInstance("PC/SC", null); else tf = TerminalFactory.getInstance("JcopEmulator", null);
            ...
            for (CardTerminal terminal : terminals.list(CardTerminals.State.ALL)) {
                try {
                    ...
                    GlobalPlatformService service = (sdAID == null) ? new GlobalPlatformService(channel) : new GlobalPlatformService(sdAID, channel);
                    service.addAPDUListener(service);
                    service.open();
                    service.setKeys(keySet, keys[0], keys[1], keys[2], diver);
                    int neededExtraSize = apduMode == APDU_CLR ? 0 : (apduMode == APDU_MAC ? 8 : 16);
                    if (loadSize + neededExtraSize > defaultLoadSize) {
                        loadSize -= neededExtraSize;
                    }
                    service.openSecureChannel(keySet, 0, GlobalPlatformService.SCP_ANY, apduMode, gemalto);
                    if (deleteAID.size() > 0) {
                    ...
                    if (listApplets) {
                        ...
                        for (AIDRegistryEntry e : registry) {
                            ...
                            System.out.print("AID: " + GPUtil.byteArrayToString(aid.getBytes()) + spaces + " " + GPUtil.byteArrayToReadableString(aid.getBytes()) + spaces2);
                            System.out.format(" %s LC: %d PR: 0x%02X\n", e.getKind().toShortString(), e.getLifeCycleState(), e.getPrivileges());
                            for (AID a : e.getExecutableAIDs()) {
                                numSpaces = (15 - a.getLength()) * 3;
                                spaces = "";
                                for (int i = 0; i < numSpaces; i++) spaces = spaces + " ";
                                System.out.println("     " + GPUtil.byteArrayToString(a.getBytes()) + spaces + " " + GPUtil.byteArrayToReadableString(a.getBytes()));
                            }
                            System.out.println();
                        }
                    }
            ...
            if (e.getCause().getMessage().equalsIgnoreCase("SCARD_E_NO_READERS_AVAILABLE")) System.out.println("No smart card readers found"); else e.printStackTrace();
        } catch (NoSuchAlgorithmException e) {
            if (e.getCause().getMessage().equalsIgnoreCase("SCARD_E_NO_SERVICE")) System.out.println("No smart card readers found (PC/SC service not running)"); else e.printStackTrace();
        } catch (Exception e) {
