...
/*
 * This file is part of JOP, the Java Optimized Processor see <http://www.jopdesign.com/>
 * 
 ...
 * and Harvey, Timothy J. and Kennedy, Ken).
 */
public class Dominators<V, E> {
  ...
  protected int getOrder(final V vertex) {
    return preOrderMap.get(vertex);
  }

  protected V getIDom(final V vertex) {
    if (idom == null)
      computeDominators();
   ...
   */
  public Dominators(final DirectedGraph<V, E> g, final V entry) {
    this(g, dfsPreOrder(g, entry));
  }

  private static <V, E> Vector<V> dfsPreOrder(final DirectedGraph<V, E> g, final V exit) {
    final DepthFirstIterator<V, E> iter = new DepthFirstIterator<V, E>(g, exit);
    iter.setCrossComponentTraversal(false);
    final Vector<V> trav = new Vector<V>();
    while (iter.hasNext()) {
      trav.add(iter.next());
    }
    return trav;
   ...
   * 
   * @param g the graph
   * @param preOrder a pre-order DFS traversal of the graph. Its first node is the entry point of
   *        the graph.
   */
  public Dominators(final DirectedGraph<V, E> g, final Vector<V> preOrder) {
    this.graph = g;
  ...
  protected void computeDominators() {
    ...
    final V firstElement = vertexPreOrder.firstElement();
    ...
    do {
      changed = false;
      for (final V v : vertexPreOrder) {
        if (v.equals(firstElement))
          continue;
        final V oldIdom = getIDom(v);
        V newIdom = null;
        for (final E edge : graph.incomingEdgesOf(v)) {
          final V pre = graph.getEdgeSource(edge);
          if (getIDom(pre) == null)
  ...
  /**
   * Get the table of immediate dominators. Note that by convention, the graph's entry is dominated
   * by itself (so <code>IDOM(n)</code> is a total function).</br>
   ...
   * @param dominator
   * @param dominated
   * @return true, if <code>dominator</code> dominates <code>dominated</code> w.r.t to the entry
   *         node
   */
  public boolean dominates(final V dominator, final V dominated) {
    computeDominators();
  ...
  public Set<V> getStrictDominators(final V n) {
    computeDominators();
    final Set<V> strictDoms = new HashSet<V>();
    V dominated = n;
    V iDom = getIDom(n);
