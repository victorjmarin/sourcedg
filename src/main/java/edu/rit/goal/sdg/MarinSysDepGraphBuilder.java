...
public class MarinSysDepGraphBuilder extends AbstractSysDepGraphBuilder {
	...
	final Vertex enterVtx = new Vertex(vtxId++, VertexType.ENTRY, methodName, methodName);
	...
	if (returnType != PrimitiveType.VOID) {
	    ...
	    final Vertex resultOutVtx = new Vertex(vtxId++, VertexType.FORMAL_OUT, resultOutVtxName, resultOutVtxName);
	    ...
	    final Vertex resultInVtx = new Vertex(vtxId++, VertexType.FORMAL_IN, resultInVtxName, resultInVtxName);
	...
	if (params != null) {
		...
		final Vertex formalParamVtx = new Vertex(vtxId++, VertexType.FORMAL_IN, fpName, methodName + fpName);
		formalParamVtx.setAssignedVariable(fpName);
	...
	final Vertex declVtx = new Vertex(vtxId++, VertexType.ASSIGN, variableDecl.toString(), lookupId(varName));
	sdg.addVertex(declVtx);
	declVtx.setAssignedVariable(varName);
    ...
    @Override
    public List<Vertex> basicForStmt(final BasicForStmt basicForStmt, final SysDepGraph sdg, final boolean isNested) {
	...
	final Vertex condVtx = new Vertex(vtxId++, VertexType.CTRL, iterable.toString());
    ...
    public List<Vertex> forStmt(final List<Stmt> init, final Expr cond, final List<Stmt> update, final List<Stmt> body,
	    final SysDepGraph sdg, final boolean isNested) {
	// Condition
	final Vertex condVtx = new Vertex(vtxId++, VertexType.CTRL, cond.toString());
	sdg.addVertex(condVtx);
	// Loop
	sdg.addEdge(condVtx, condVtx, EdgeType.CTRL_TRUE);
	...
	final Vertex condVtx = new Vertex(vtxId++, VertexType.CTRL, condition.toString());
	sdg.addVertex(condVtx);
	// Then branch
	...
	for (final SwitchBlockStmtGroup b : blocks) {
	    ...
	    final Vertex condVtx = new Vertex(vtxId++, VertexType.CTRL, vtxLabel);
	    sdg.addVertex(condVtx);
	...
	final Vertex assignVtx = new Vertex(vtxId++, VertexType.ASSIGN, arrayAccessAssignment.toString(),
		lookupId(assignedVar));
	...
	final Vertex assignVtx = new Vertex(vtxId++, VertexType.ASSIGN, assignment.toString(), lookupId(outVar));
	final Set<String> inVars = assignment.getInVars();
	sdg.addVertex(assignVtx);
	assignVtx.setAssignedVariable(outVar);
	...
	if (isLoopBody && VisitorUtils.isSelfAssignment(assignment)) {
	    sdg.addEdge(assignVtx, assignVtx, EdgeType.DATA);
	}
	if (!isNested) {
	    // Method entry dependency
	    notNestedStmtEdge(assignVtx, sdg);
    ...
    public List<Vertex> methodInvocation(final MethodInvocation methodInvocation, final SysDepGraph sdg,
	    final boolean isNested) {
	final Vertex invocationVtx = new Vertex(vtxId++, VertexType.CALL, methodInvocation.toString());
	...
	if (inVars != null) {
		...
		if (formalParam != null) {
		    ...
		    final Vertex actualParam = new Vertex(vtxId++, VertexType.ACTUAL_IN, paramName, paramName);
	...
	// Actual out
	final Vertex actualOutVtx = new Vertex(vtxId++, VertexType.ACTUAL_OUT, outVar, lookupId(outVar));
	...
	// Actual in
	final Vertex actualInVtx = new Vertex(vtxId++, VertexType.ACTUAL_IN, outVar);
	...
	final Vertex breakVtx = new Vertex(vtxId++, VertexType.BREAK, "break");
    ...
    @Override
    public List<Vertex> continueStmt(final ContinueStmt continueStmt, final SysDepGraph sdg, final boolean isNested) {
	final Vertex continueVtx = new Vertex(vtxId++, VertexType.CONTINUE, "continue");
	...
	final Vertex returnVtx = new Vertex(vtxId++, VertexType.RETURN, returnedExpr.toString());
	...
	sdg.addEdge(returnVtx, getCurrentResultVertex(), EdgeType.DATA);
	...
	final Vertex vtx = new Vertex(vtxId++, VertexType.ASSIGN, expr.toString(), lookupId(lookupId));
	sdg.addVertex(vtx);
	if (!isNested) {
	    // Method entry dependency
	...
	if (isLoopBody) {
	    sdg.addEdge(vtx, vtx, EdgeType.DATA);
	}
    ...
    protected List<Vertex> ctrlStructureTrue(final Vertex conditionVtx, final List<Stmt> body, final SysDepGraph sdg,
	    final boolean isNested, final boolean isLoopBody) {
	return ctrlStructureTrue(conditionVtx, body, sdg, false, null, isNested, isLoopBody);
    }

    protected List<Vertex> ctrlStructureTrue(final Vertex conditionVtx, final List<Stmt> body, final SysDepGraph sdg,
	    final Function<Void, Void> f, final boolean isNested, final boolean isLoopBody) {
	return ctrlStructureTrue(conditionVtx, body, sdg, false, f, isNested, isLoopBody);
    }

    protected List<Vertex> ctrlStructureTrue(final Vertex conditionVtx, final List<Stmt> body, final SysDepGraph sdg,
	    final boolean isDoStmt, final Function<Void, Void> f, final boolean isNested, final boolean isLoopBody) {
    ...
    private void dataDependencies(final Vertex vertex, final Set<String> deps, final SysDepGraph sdg,
	    final boolean isNested) {
	vertex.setReadingVariables(deps);
	for (final String s : deps) {
		...
		final Vertex initialStateVtx = new Vertex(vtxId++, VertexType.INITIAL_STATE, s);
