...
public class PDGBuilder
{
	...
	normalizedCu = cu;
	cdgBuilder = new CDGBuilder(
		cu);
	cdgBuilder.build(config);
	pdg = cdgBuilder.getCDG();
	if (config.isInterproceduralCalls())
	    computeInterProceduralCalls(cdgBuilder.getMethodParams(), cdgBuilder.getCalls(), cdgBuilder
		    .getMethodFormalOut());
    ...
    private void computeInterProceduralCalls(final HashMap<String, Pair<Vertex, List<Vertex>>> methodParams,
	    final HashMap<String, Set<Pair<Vertex, List<Vertex>>>> calls, final HashMap<Vertex, Vertex> methodFormalOut)
    {
	for (final Entry<String, Set<Pair<Vertex, List<Vertex>>>> e : calls.entrySet())
	{
		...
		pdg.addEdge(caller, callee, new Edge(
			caller,
			callee,
			EdgeType.CALL));
		for (int i = 0; i < callPair.b.size(); i++)
		{
		    ...
		    pdg.addEdge(callArg, defParam, new Edge(
			    callArg,
			    defParam,
			    EdgeType.PARAM_IN));
		}
		...
		pdg.addEdge(formalOut, actualOut, new Edge(
			formalOut,
			actualOut,
			EdgeType.PARAM_OUT));
	    }
	...
	while (changes)
	{
		...
		final Set<Vertex> diff = new HashSet<>(
			n.getIn());
