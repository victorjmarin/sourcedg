...
public class CDGBuilder
{
	...
	for (final MethodDeclaration m : methods)
	    _build(m);
	final List<ClassOrInterfaceDeclaration> classes = n
		.findAll(ClassOrInterfaceDeclaration.class, t -> !t.equals(n));
	...
	methodParams.put(methodName, new Pair<>(
		v,
		paramVtcs));
	return result;
    }
	...
	ControlFlow exitFlow = new ControlFlow(
		exit,
		exit);
	...
	if (n.getExpression().isPresent())
	{
	    ...
	    else addEdge(EdgeType.DATA, v, formalOut);
	}
	return new ControlFlow(
		v,
		CFGBuilder.EXIT);
	...
	cdg.addVertex(v);
	inScope.add(v);
	final ControlFlow result = new ControlFlow(
		v,
		CFGBuilder.EXIT);
	...
	addEdge(EdgeType.CTRL_TRUE, v, v);
	loopStack.pop();
	final List<Vertex> oldScope = new ArrayList<>(
		inScope);
	popScope();
    ...
    private ControlFlow args(final Vertex v, final NodeList<Expression> args, final String name, final Expression scope)
    {
	final List<ControlFlow> result = new ArrayList<>();
	final List<Vertex> paramVtcs = new ArrayList<>();
	result.add(new ControlFlow(
		v,
		v));
    ...
    {
	cdg.addEdge(source, target, new Edge(
		source,
		target,
		type));
    }
